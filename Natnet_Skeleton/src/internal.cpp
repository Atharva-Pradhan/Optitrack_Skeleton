...
void Internal::Info(NatNetClient* g_pClient, ros::NodeHandle &n)
{
    ...
    else
    {
        ...
        for(int i=0; i < pDataDefs->nDataDescriptions; i++)
        {
            ...
            // {
            //     // Skeleton Works partially DO NOT TAMPER
            //     // ROS_INFO("A %s", pDataDefs);

            //     sSkeletonDescription* pS = pDataDefs->arrDataDescriptions[i].Data.SkeletonDescription;
            //     sRigidBodyDescription* pRB = pDataDefs->arrDataDescriptions[i].Data.RigidBodyDescription;
            //     ROS_INFO("Skeleton found: %s", pS->szName);
            //     ROS_INFO_COND(rosparam.log_internals, "RigidBody ID : %d", pS->skeletonID);
            //     ROS_INFO_COND(rosparam.log_internals, "Number of Rigid Bodies : %d", pS->nRigidBodies);
            //     // ROS_INFO_COND(rosparam.log_internals, "Parent Offset : %3.2f,%3.2f,%3.2f", pRB->offsetx, pRB->offsety, pRB->offsetz);
                
            //     // Creating publisher for the rigid bodies if found any
            //     for(int k=0; k<pS->nRigidBodies; k++)
            //     // for(int k=0; k<1; k++)
            //     {
            //         std::string body_name(pS->RigidBodies[k].szName);
            //         if(rosparam.pub_rigid_body)
            //         {                    
            //             this->ListRigidBodies[pS->RigidBodies[k].ID] = body_name;
            //             this->RigidbodyPub[pS->RigidBodies[k].szName] = n.advertise<geometry_msgs::PoseStamped>(body_name+"/pose", 50);
            //         }
            //         if ( pS->RigidBodies[k].MarkerPositions != NULL && pS->RigidBodies[k].MarkerRequiredLabels != NULL )
            //         {
            //         // ROS_INFO("A %d", rosparam.pub_skeleton);
            //             for ( int markerIdx = 0; markerIdx < pS->RigidBodies[k].nMarkers; ++markerIdx )
            //             {
            //                 const MarkerData& markerPosition = pS->RigidBodies[k].MarkerPositions[markerIdx];
            //                 const int markerRequiredLabel = pS->RigidBodies[k].MarkerRequiredLabels[markerIdx];
            //                 // Creating publisher for the markers of the skeleton
            //                 if(rosparam.pub_rigid_body_marker)
            //                     this->RigidbodyMarkerPub[std::to_string(pS->RigidBodies[k].ID)+std::to_string(markerIdx+1)] = n.advertise<geometry_msgs::PointStamped>(body_name+"/marker"+std::to_string(markerIdx)+"/pose", 50);
            //                 ROS_INFO_COND(rosparam.log_internals,  "\tMarker #%d:", markerIdx );
            //                 ROS_INFO_COND(rosparam.log_internals,  "\t\tPosition: %.2f, %.2f, %.2f", markerPosition[0], markerPosition[1], markerPosition[2]);
            ...
            else if(pDataDefs->arrDataDescriptions[i].type == Descriptor_Skeleton)
            {
                ...
                // Creating publisher for the skeleton if found any
                
                std::string body_name(pS->szName);
                if(rosparam.pub_skeleton)
                {                    
                    this->ListSkeletons[pS->skeletonID] = body_name;
                    this->SkeletonPub[pS->szName] = n.advertise<geometry_msgs::PoseArray>(body_name+"/pose", 50);
                }
                for(int k=0; k<pS->nRigidBodies; k++)
                {
                    if ( pS->RigidBodies[k].MarkerPositions != NULL && pS->RigidBodies[k].MarkerRequiredLabels != NULL )
...
{   
    ...
    // ROS_INFO("Number of Rigid Bodies %d",data->nRigidBodies);
    // ROS_INFO("Number of Skeletons %d",data->nSkeletons);
    ...
    for(i=0; i < data->nSkeletons; i++)
        {
            ...
            {
                for(j=0; j < data->Skeletons[i].nRigidBodies; j++)
                {
                    // ROS_INFO("At %d th RigidBody in Skeleton %d has data %f",j,i,data->Skeletons[i].RigidBodyData[j].x);
                    ROS_INFO_COND(internal.rosparam.log_frames, "Rigid Body [ID=%d  Error=%3.2f]", data->Skeletons[i].RigidBodyData[j].ID, data->Skeletons[i].RigidBodyData[j].MeanError);
                    ROS_INFO_COND(internal.rosparam.log_frames, "x\ty\tz\tqx\tqy\tqz\tqw");
                    ROS_INFO_COND(internal.rosparam.log_frames, "%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f",
                    data->Skeletons[i].RigidBodyData[j].x, data->Skeletons[i].RigidBodyData[j].y, data->Skeletons[i].RigidBodyData[j].z,
                    data->Skeletons[i].RigidBodyData[j].qx, data->Skeletons[i].RigidBodyData[j].qy, data->Skeletons[i].RigidBodyData[j].qz, data->Skeletons[i].RigidBodyData[j].qw);
                }
                PubSkeletonPose(data->Skeletons[i], internal);
            }
            ROS_INFO_COND(internal.rosparam.log_frames, "Skeletons [ID=%d]", data->Skeletons[i].skeletonID);
        }
...
void Internal::PubSkeletonPose(sSkeletonData &data, Internal &internal)
{
    int j=0;
    // Creating a msg to put data related to the skeleton 
    geometry_msgs::PoseArray msgSkeletonPose;

    msgSkeletonPose.header.frame_id = "world";
    msgSkeletonPose.header.stamp = ros::Time::now();

    geometry_msgs::Pose p;

    for (j=0; j<data.nRigidBodies; j++)
    {
        p.position.x = data.RigidBodyData[j].x;
        p.position.y = data.RigidBodyData[j].y;
        p.position.z = data.RigidBodyData[j].z;
        p.orientation.x = data.RigidBodyData[j].qx;
        p.orientation.y = data.RigidBodyData[j].qy;
        p.orientation.z = data.RigidBodyData[j].qz;
        p.orientation.w = data.RigidBodyData[j].qw;

        // Put every pose in a vector
        msgSkeletonPose.poses.push_back(p);
    }    
    internal.SkeletonPub[internal.ListSkeletons[data.skeletonID]].publish(msgSkeletonPose);
    ...
    msgTFRigidBodies.header.stamp = ros::Time::now();
    msgTFRigidBodies.header.frame_id = "world";
    msgTFRigidBodies.child_frame_id = internal.ListSkeletons[data.skeletonID];

    int k = 0;
    msgTFRigidBodies.transform.translation.x = data.RigidBodyData[k].x;
    msgTFRigidBodies.transform.translation.y = data.RigidBodyData[k].y;
    msgTFRigidBodies.transform.translation.z = data.RigidBodyData[k].z;
    msgTFRigidBodies.transform.rotation.x = data.RigidBodyData[k].qx;
    msgTFRigidBodies.transform.rotation.y = data.RigidBodyData[k].qy;
    msgTFRigidBodies.transform.rotation.z = data.RigidBodyData[k].qz;
    msgTFRigidBodies.transform.rotation.w = data.RigidBodyData[k].qw;
    tfRigidBodies.sendTransform(msgTFRigidBodies);
}
